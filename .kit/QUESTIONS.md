...
| P1 | Does timeout-filtered sequential execution (skip entries where barrier unlikely to resolve before day end) improve per-trade expectancy toward $5.00 barrier-hit maximum? | Answered | — | — | Observed $2.50/trade vs $5.00 barrier-hit theoretical — 50% timeout dilution is the largest single drag. Filtering is zero-model-change improvement path. Determines if sequential expectancy can reach $4+/trade, reducing account minimum. | timeout-filtered-sequential |
| P2 | Does volume-flow conditioned entry (enter during high-volume periods) reduce timeout fraction below 41% and improve per-trade expectancy? | Answered | — | — | Timeout fraction invariant at 41.3% across all time-of-day cutoffs — timeouts driven by volume horizon, not clock. Volume-flow conditioning targets the actual mechanism. | volume-flow-conditioned-entry |
...
| Does time-of-day entry filtering improve sequential expectancy toward $5.00 barrier-hit maximum? | REFUTED | NO — timeout fraction is invariant at ~41.3% across ALL 7 time-of-day cutoffs (range 0.21pp). Timeouts are driven by volume horizon, not clock time. Best cutoff=270 achieves $3.02/trade (+$0.52) and $34K min account, but through hold-skip restructuring (66%→34%), not timeout avoidance. Neither $3.50/trade nor $30K targets met. Calmar +15%, ROC 216%→249%. SC-S4 U-shape: intermediate-late-day trades (5-5.75h) are actually BETTER than average. | `.kit/results/timeout-filtered-sequential/analysis.md` |
| Does volume-flow conditioned entry reduce timeout fraction below 41% and improve per-trade expectancy? | REFUTED | NO — all 5 volume/activity features show 20pp diagnostic range but sequential simulation reduces timeouts by max 1.76pp (91% evaporation). Hold-skip (66.1%) already self-selects for high-activity bars. Best stacked config: $3.10/trade, $33K min account, Calmar 2.59 — marginal over cutoff=270 alone (+$0.08/trade). SC-2/3/5 all fail. Timeout ~41.3% is a structural constant of the volume horizon mechanism, invariant to both time-of-day and volume/activity gating. Entry-time filtering exhausted as an intervention class. | `.kit/results/volume-flow-conditioned-entry/analysis.md` |
