...
| Does timeout-filtered sequential execution improve per-trade expectancy toward $5.00 barrier-hit maximum? | REFUTED (Outcome B) | Timeout premise was WRONG: only 0.40% of trades are timeouts (not 50% dilution). Best cutoff=270 achieves $3.02/trade (+21%) and $34K min_acct (-29%), but misses $3.50 and $30K targets. Expectancy is U-shaped (SC-S4 fails) — moderate cutoffs hurt. Drawdown reduction is largely mechanical (28% fewer trades → 29% less DD). ROC improves (249% vs 216%) but absolute daily PnL drops 18.4%. The sequential selection mechanism (hold-skip 66.1%) is the real economic driver, not timeouts. | `.kit/results/timeout-filtered-sequential/analysis.md` |
