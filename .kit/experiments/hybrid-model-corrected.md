...
- [x] TICK_SIZE normalization verified (channel 0 range [-22.5, 22.5], 100% half-tick quantized — see normalization_verification.txt)
- [x] Per-day z-scoring verified (channel 1 per-day mean≈0, std≈1 — max dev 4.5e-16)
- [x] Validation split verified (no test data in val set, day boundaries confirmed)
- [x] Architecture verified (12,128 params exactly)
- [x] MVE gate passed (fold 5 train R²=0.190 > 0.05, test R²=0.140 within 0.005 of 9D's 0.135)
- [x] Full 5-fold CNN training completed with per-fold R² reported (mean=0.089)
- [x] CNN R² comparison with 9D proper-validation reference (all 5 folds within ±0.015)
- [x] Hybrid XGBoost training completed (5 folds, mean acc=0.419)
- [x] GBT-only ablation completed (GBT-book and GBT-nobook, 5 folds each)
- [ ] CNN-only ablation completed — NOT RUN (protocol deviation: replaced with GBT-book ablation)
- [x] PnL and cost sensitivity computed (3 scenarios × 3 configs)
- [x] Feature importance top-10 reported (return_5 ranked ~26th, NOT top-3)
- [x] Label distribution per fold reported
- [x] Label=0 simplification impact reported (0 impact — wins+losses=trade_count for all configs)
- [x] Hybrid vs GBT-only delta reported (+0.4pp acc, +$0.075 exp vs GBT-nobook)
- [ ] Hybrid vs CNN-only delta reported — NOT AVAILABLE (CNN-only not run)
- [x] analysis.md written with all required sections
- [x] All SC-1 through SC-8 evaluated explicitly (7/9 PASS, SC-4 and SC-5 FAIL — expectancy and PF below threshold)
