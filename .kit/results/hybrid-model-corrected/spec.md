...
2. **XGBoost classification accuracy >= 0.38** on 3-class triple barrier labels, with the hybrid feature set (CNN 16-dim embedding + 20 non-spatial features) outperforming GBT-only (all non-book features, no CNN).
...
1. **Model configuration** (2 levels):
...
For each fold k:
1. Split training days into 80% train / 20% validation **by day boundaries**.
   - Fold 1 (4 train days): 3 train + 1 val
   - Fold 2 (7 train days): 5-6 train + 1-2 val
   - Fold 3 (10 train days): 8 train + 2 val
   - Fold 4 (13 train days): 10 train + 3 val
   - Fold 5 (16 train days): 13 train + 3 val
2. Early stopping monitors validation loss (NOT test loss).
3. Test data is NEVER seen during training or model selection.
4. **Verification:** Print the day indices for train/val/test splits for each fold.
...
| ablation_delta_accuracy | Hybrid accuracy - GBT-only accuracy |
| ablation_delta_expectancy | Hybrid expectancy - GBT-only expectancy |
...
- [ ] **SC-7**: Cost sensitivity table produced for all 3 scenarios
...
OUTCOME D — Hybrid Equals GBT-only (SC-1+SC-2 PASS, SC-6 FAIL):
  ...
  -> The 16-dim embedding is redundant with the 20 non-spatial features.
  -> This suggests the spatial structure the CNN captures is already reflected
     in hand-crafted book features (e.g., weighted_imbalance, spread).
  -> Decision: Simplify to GBT-only architecture. Drop CNN encoder.
  -> Follow-up: Test with CNN-only (no hand-crafted features) to confirm
     whether CNN captures something distinct.
```
...
- Split fold 5 train days into 80% train / 20% val BY DAY.
...
6. Construct `fwd_return_5` target.
...
| Fold | Train Days | Val Days (last 20% of train) | Test Days |
|------|-----------|------------------------------|-----------|
| 1 | Days 1-3 | Day 4 | Days 5-7 |
| 2 | Days 1-5/6 | Days 6/7 | Days 8-10 |
| 3 | Days 1-8 | Days 9-10 | Days 11-13 |
| 4 | Days 1-10 | Days 11-13 | Days 14-16 |
| 5 | Days 1-13 | Days 14-16 | Days 17-19 |
...
1. Split by day boundaries. Reserve last 20% of train days as validation.
...
3. Train XGBoost (multi:softmax, num_class=3, same hyperparameters as 9B) on train -> tb_label.
...
For each fold, each cost scenario (optimistic $2.49, base $3.74, pessimistic $6.25):
```
...
- Produce: R3/9D comparison table, cost sensitivity table, feature importance top-10, label distribution.
...
- Max training runs: 16 (5 folds CNN + 1 MVE CNN + 5 folds XGB hybrid + 5 folds XGB GBT-only)
- Max seeds: 1 (seed=42)
...
1. **Validation split reduces effective training data.** With proper 80/20 split, folds 1-2 have very few validation days (1-2 days). Early stopping may be unreliable on small validation sets. Expected impact: higher variance on early folds, possibly lower R-squared than 9D's proper-validation results (which used fold 5 = 16 train days). This is inherent to the expanding-window design and not fixable without changing the CV strategy.
...
.kit/results/hybrid-model-corrected/
  ...
  cost_sensitivity.json          # 3 scenarios x {expectancy, pf, trade_count}
...
- [ ] PnL and cost sensitivity computed (3 scenarios)
