...
**4. Single CPCV split — ALL 3 MODEL CONFIGS (split 1 of 45) — MANDATORY before full run:**
Run split 1 for ALL three model configurations. This catches architecture bugs (like the conv2 shape mismatch that crashed the prior run after 5 hours) before committing to the full 45-split CPCV.

- **4a. E2E-CNN classification:**
  ...
  - **Gate A:** train accuracy < 0.35 → model not learning. ABORT.
  - **Gate B:** test accuracy < 0.30 → worse than random. ABORT.
  - **Gate C:** test accuracy > 0.35 → proceed.

- **4b. E2E-CNN + Features (CRITICAL — this is where the prior run crashed):**
  - Same encoder as 4a, concatenate 16-dim embedding with 20 features → Linear(36→3)
  - **Forward-pass shape assertion BEFORE training:** Run 1 batch through the model. Assert output shape is [B, 3]. If RuntimeError or shape mismatch → FIX THE BUG, do NOT proceed to full CPCV.
  - Train 1 split. Print: train accuracy, val accuracy, test accuracy.
  - **ABORT if forward pass fails or accuracy < 0.30**

- **4c. GBT-only:**
  - Train XGBoost on same split. Print accuracy.
  - **ABORT if accuracy < 0.33**

**ALL THREE configs must pass MVE on split 1 before proceeding to full CPCV. No exceptions.**
...
### Execution Strategy: Local MPS Validation → Cloud GPU Full Run

**TWO-STAGE EXECUTION — MANDATORY:**

**Stage 1 — Local MPS validation (MUST COMPLETE BEFORE Stage 2):**
- Run the FULL MVE (data loading, normalization, holdout isolation, ALL 3 model configs on split 1) locally using Apple MPS (`torch.device('mps')`)
- This validates the entire pipeline — data loading, normalization, model architecture, forward/backward pass, loss computation, prediction, metric calculation — in minutes, not hours
- **ALL 3 model configs must produce valid predictions on split 1 before Stage 2 launches**
- If ANY config fails (shape mismatch, NaN, crash), fix it locally and re-test. Do NOT send broken code to EC2.
- Set environment variable `PYTHONUNBUFFERED=1` for real-time log output.

**Stage 2 — Cloud GPU full CPCV (only after Stage 1 exits cleanly):**
- Upload the validated script to S3
- Launch on EC2 g5.xlarge (A10G) for full 45-split CPCV × 3 configs + walkforward + holdout
- The script must detect device automatically: CUDA if available, else MPS, else CPU

### GPU Requirements — NON-NEGOTIABLE

**The prior CPU run (2026-02-22) took 180-320s/split and timed out before completing all configs.** GPU is mandatory for the full experiment.
...
## MANDATORY: Bug Fixes from Prior Run (2026-02-22)

### Bug 1: Conv2 Shape Mismatch in CNN+Features Forward Pass (FATAL)

The prior run crashed in Phase 5 (E2E-CNN + Features) with:
```
RuntimeError: Given groups=1, weight of size [59, 59, 3],
expected input[1024, 2, 20] to have 59 channels, but got 2 channels instead
```
**Root cause:** The `forward()` method passes the raw book input `book_x` (shape [B, 2, 20]) directly to `conv2` instead of passing the output of `conv1` (shape [B, 59, 20]). The CNN+Features model must share the EXACT same encoder as the E2E-CNN model — conv1 output feeds into conv2, not the raw input.

**Fix:** In the CNN+Features forward pass, ensure the conv layers are applied sequentially:
```
x = relu(bn1(conv1(book_x)))   # [B, 2, 20] → [B, 59, 20]
x = relu(bn2(conv2(x)))         # [B, 59, 20] → [B, 59, 20]  ← use x, NOT book_x
x = adaptive_avg_pool(x)        # → [B, 59]
embedding = relu(fc1(x))        # → [B, 16]
combined = cat([embedding, features], dim=1)  # → [B, 36]
logits = fc2(combined)           # → [B, 3]
```

**Verification:** Before running Phase 5, do a forward pass sanity check on 1 batch: assert output shape is [B, 3] and no RuntimeError. ABORT if shape mismatch persists.

### Bug 2: No Incremental Result Saving (DATA LOSS RISK)

The prior run completed 45 CNN splits (~5 hours of GPU compute) but crashed in Phase 5, losing all downstream results. Only `purge_audit.json` and `cpcv_split_results.json` were saved.

**Fix — MANDATORY incremental saving protocol:**
1. **After EACH CPCV split completes**, append the split result to the JSON file on disk immediately (do not accumulate in memory and write once at the end).
2. **After EACH phase completes** (Phase 4, 5, 6, 7, 8, 9), write all phase results to their respective output files BEFORE starting the next phase.
3. **Use `json.dump()` with `flush=True`** (or `f.flush(); os.fsync(f.fileno())`) to ensure data hits disk.
4. **Upload intermediate results to S3 after each phase** — use `boto3` to upload the phase results directory. This way, even if the instance crashes, completed phases are preserved in S3.
5. **Print progress to stdout with `flush=True`** on every print statement (or set `PYTHONUNBUFFERED=1` environment variable) so logs are visible in real time, not buffered.

**Result file protocol per phase:**
- Phase 4 complete → write `e2e_cnn/cpcv_split_results.json` → upload to S3
- Phase 5 complete → write `e2e_cnn_features/cpcv_split_results.json` → upload to S3
- Phase 6 complete → write `gbt_only/cpcv_split_results.json` → upload to S3
- Phase 7 complete → write `cpcv/path_results.json`, `cpcv/pbo.json` → upload to S3
- Phase 8 complete → write `walkforward/fold_results.json` → upload to S3
- Phase 9 complete → write `holdout/results.json` → upload to S3
