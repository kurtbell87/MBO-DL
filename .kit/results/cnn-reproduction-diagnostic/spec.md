...
- [ ] **SC-1**: mean_cnn_r2_h5 ≥ 0.10 across 5 folds (CNN reproduces ≥ 76% of R3's signal)
- [ ] **SC-2**: No fold produces train R² < 0.05 at h=5 (CNN can fit training data — the Phase B failure mode is eliminated)
- [ ] **SC-3**: aggregate_expectancy_base ≥ $0.50/trade *(conditional: only evaluated if SC-1 passes)*
- [ ] **SC-4**: Hybrid outperforms GBT-only on accuracy OR expectancy — at least one *(conditional: only evaluated if SC-1 passes)*
- [ ] **SC-5**: No sanity check failures
...
- [ ] Step 1 MVE check executed (fold 5 only) with diagnostics reported
- [ ] Input normalization verified (sample values printed, channel 0 = raw ticks confirmed)
- [ ] Architecture verified (param count logged, matches R3 within 5%)
- [ ] Full 5-fold Step 1 completed with per-fold R² reported
- [ ] Step 1 gate evaluated: PASS (≥ 0.10) / MARGINAL (0.05–0.10) / FAIL (< 0.05)
- [ ] R3 comparison table (this experiment vs R3, per-fold)
- [ ] If Step 1 PASS/MARGINAL: Step 2 hybrid integration completed
- [ ] If Step 1 PASS/MARGINAL: GBT-only ablation completed
- [ ] If Step 1 PASS/MARGINAL: PnL and cost sensitivity computed
- [ ] If Step 1 PASS/MARGINAL: Hybrid vs GBT-only delta reported
- [ ] analysis.md written with all required sections
- [ ] Deviations log completed (target: empty)
- [ ] All SC-1 through SC-5 evaluated explicitly
